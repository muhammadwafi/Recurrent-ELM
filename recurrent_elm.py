...
class RELMClassifier:
    ...
    def __init__(
        ...
        if activation_method not in AVAILABLE_AF:
            raise ValueError(
    ...
    def activation(self, h_init) -> np.ndarray:
        if self.activation_method == "sigmoid":
            ...
            result = af.hyperbolic_tangen(h_init)
    ...
    def get_matrix_delay(self, X, labels) -> pd.DataFrame:
        ...
        delay_df = pd.DataFrame(
            reshaped,
            columns=[f"Xd{i}" for i in range(1, self.r+1)]
        )

        return delay_df

    def score(self, y_pred, y_test) -> str:
